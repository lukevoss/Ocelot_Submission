...
def generate_predictions(model, data_directory, output_directory, iou):
    """
    ...
    for filename in os.listdir(data_directory):
        if filename.endswith('.jpg'):
            ...
            results = model(image_path, iou=iou)
            ...
            pred_class = np.array(boxes.cls.tolist()).astype(int).tolist()
...
def convert_csv_to_json(csv_file_path, output_path, raw_or_corrected="corrected"):
    ...
    with open(f"{output_path}\\{raw_or_corrected}_predictions_valid.json", "w") as g:
        json.dump(pred_json, g)
        print(f"JSON file saved in {output_path}\{raw_or_corrected}_predictions.json")
...
def plot_predictions(image_path, segmentation_path, gt_all, pred_all, filename, metadata_path):
    # Load the image
    ...
    # Loop through the annotations and plot each cell as a circle
    for annotation in gt_all:
        x, y, cell_class, confidence = annotation
        if cell_class == 1: # BC (Background Cell)
            color = 'green'
        elif cell_class == 2: # TC (Tumor Cell)
            color = 'red'
        else:
            color = 'black'
    ...
    # Loop through the predictions and plot an x above each cell
    for prediction in pred_all:
        x, y, cell_class, confidence = prediction
        if cell_class == 1:
            color = 'green'
        elif cell_class == 2:
            color = 'red'
            ...
            color = 'black'
        ax.plot(x, y, marker='x', markersize=10, color=color)

    with open(metadata_path) as f:
        data = json.load(f)

    meta_pair = data['sample_pairs'][str(filename[:-4])]

    # Open the segmentation mask using PIL
    seg = Image.open(segmentation_path)

    # Convert the segmentation mask to a numpy array
    pred_mask_array = np.array(seg)
    ...
    pred_mask_array[pred_mask_array == 255] = 3
    # converting the segmentation mask back to a pil image
    ...
    # Set the values of the colormap
    cmap.set_over('white') # For unknown pixels
    cmap.set_under('black') # For background pixels

    # Create a new figure and plot the image
    ax.imshow(pred_mask_array, cmap=cmap, alpha=0.5)
